Select
    concat(cont.name, ' - ', user.name) As Auditor,
...
From
    ...
    jm4_users user On user.id = assi.ia_user_sysid Left Join
    jm4_contact_details cont On cont.id = assi.ia_assi_customer
Where
    Case
        When Month(Date(Now())) Not In ('10', '11', '12')
        Then (assi.ia_assi_year = Year(Date(Now())) And cate.ia_assicate_private = '0') Or
            (assi.ia_assi_year < Year(Date(Now())) And assi.ia_assi_priority < 9 And cate.ia_assicate_private = '0') Or
            (assi.ia_assi_finishdate Between Concat(Year(Date_Add(Now(), Interval -1 Year)), '1001') And
            Concat(Year(Date(Now())), '0930') And cate.ia_assicate_private = '0')
        Else (assi.ia_assi_year = Year(Date_Add(Now(), Interval 1 Year)) And cate.ia_assicate_private = '0') Or
            (assi.ia_assi_year < Year(Date_Add(Now(), Interval 1 Year)) And cate.ia_assicate_private = '0' And
            assi.ia_assi_priority < 9) Or
            (assi.ia_assi_finishdate Between Concat(Year(Date(Now())), '1001') And Concat(Year(Date_Add(Now(),
            Interval 1 Year)), '0930') And cate.ia_assicate_private = '0')
    End
Group By
    cont.name,
    user.name,
